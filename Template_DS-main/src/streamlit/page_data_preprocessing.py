...
def page_data_preprocessing(df_hist, df_villes):
    ...
    st.write("#### :blue[**4.2. Pré-processing par colonne**]")   
    ...
    with tab2:  # Location
        ...
        st.write(" - Les coordonnées géographiques de la localité : Longitude et Latitude;")
    ...
    st.write("Afin de mutualiser les traitements, nous avons créé des fonctions de complétion des données : ")
    ...
    st.write("   ***- f_maj_humidity*** : Mise à jour des données manquantes sur le colonne Humidity. Si Raintoday = 1 , Alors Humidity = 100,   sinon  par la moyenne des humidité de la semaine pour le même climat ")  
    st.write("   ***- f_maj_rainfall*** : Mise à jour des données manquantes sur le colonne Rainfall. Si Raintoday = 0 , Alors Rainfall = 0,  sinon  par la moyenne des rainfall de la semaine pour le même climat ")  
